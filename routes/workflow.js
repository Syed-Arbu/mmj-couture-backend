...
router.get('/state',requireAuth,async(req,res)=>{try{const u=user(req),admin=await isAdminVerified(req);const requests=(await db.query(admin?'SELECT * FROM bill_edit_requests ORDER BY id DESC':'SELECT * FROM bill_edit_requests WHERE staff_id=$1 ORDER BY id DESC',admin?[]:[u.id])).rows.map(r=>({id:r.id,orderId:r.order_no,staffId:r.staff_id,staffName:r.staff_name,status:r.status,when:r.created_at,approvedAt:r.approved_at,usedAt:r.used_at}));const rows=(await db.query(admin?'SELECT * FROM staff_messages ORDER BY created_at ASC,id ASC':'SELECT * FROM staff_messages WHERE recipient_staff_id=$1 ORDER BY created_at ASC,id ASC',admin?[]:[u.id])).rows;const am=await attachmentMap(rows.map(r=>r.id));const messages=rows.map(r=>({id:r.id,staffId:r.recipient_staff_id||r.staff_id,staffName:r.staff_name||r.recipient_staff_id,senderId:r.sender_id||r.staff_id,senderName:r.sender_name||r.staff_name||r.staff_id,senderRole:r.sender_role||'staff',text:r.message||'',when:r.created_at,attachments:am.get(r.id)||[]}));const history=admin?(await db.query('SELECT * FROM bill_history ORDER BY id DESC LIMIT 250')).rows.map(r=>({id:r.id,orderId:r.order_no,changedBy:r.changed_by,when:r.changed_at,oldBill:r.old_bill,newBill:r.new_bill})):[];const staffDirectory=admin?(await db.query("SELECT login_id,name FROM users WHERE role='staff' ORDER BY COALESCE(name,login_id),login_id")).rows.map(r=>({id:r.login_id,name:r.name||r.login_id})):[];res.json({requests,messages,history,staffDirectory,customFields:await metaGet('custom_bill_fields',{client:[],body:[],length:[],garment:[],material:[],trial:[],billing:[],payment:[]}),customSections:await metaGet('custom_bill_sections',[]),relationshipOptions:await metaGet('customer_relationship_options',['New','Existing','Regular']),adminNotes:admin?await metaGet('admin_private_notes',''):''})}catch(e){console.error(e);res.status(500).json({error:'Could not load workflow data'})}});
...
router.put('/config',requireAuth,async(req,res)=>{try{if(!(await isAdminVerified(req)))return res.status(403).json({error:'Admin only'});if(req.body.customFields!==undefined)await metaSet('custom_bill_fields',req.body.customFields);if(req.body.customSections!==undefined)await metaSet('custom_bill_sections',req.body.customSections);if(req.body.relationshipOptions!==undefined){const opts=(Array.isArray(req.body.relationshipOptions)?req.body.relationshipOptions:[]).map(x=>String(x||'').trim()).filter(Boolean);await metaSet('customer_relationship_options',opts.length?opts:['New','Existing','Regular'])}res.json({ok:true})}catch(e){console.error(e);res.status(500).json({error:'Could not save configuration'})}});
